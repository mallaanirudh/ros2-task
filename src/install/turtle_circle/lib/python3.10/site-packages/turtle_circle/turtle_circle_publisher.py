...
class TurtleGoal(Node):
    def __init__(self):
        ...
        # Waypoints: go to (5.5,8.0), then draw circle, then go to (8.0,8.0)
    ...
    def move(self, target):
        if target == 'circle':
            ...
            # do NOT block here — add a callback instead
            future.add_done_callback(self.circle_done_callback)
            return

        # Otherwise, normal pose-based movement
        ...
        if distance > 0.1:
            ...
            self.current_target_index += 1  # move to next target

        self.pub.publish(twist)

    def circle_done_callback(self, future):
        """Callback executed once the circle service finishes."""
        try:
